...
def _parse_chat_top_logprobs(response):
    dict_probs = {}
    try:
        logprobs_content = response.choices[0].logprobs
        if logprobs_content and getattr(logprobs_content, "content", None):
            for item in logprobs_content.content[0].top_logprobs or []:
