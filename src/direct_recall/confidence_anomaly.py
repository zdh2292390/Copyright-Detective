...
class TokenLogprob:
    ...
    Plain class to avoid Python 3.13 dataclass sys.modules lookup during import.
    """
    __slots__ = ("token", "logprob", "linear_prob", "is_punctuation", "is_common_word", "is_whitespace")
...
class ConfidenceSpike:
    ...
    Plain class to avoid Python 3.13 dataclass sys.modules lookup during import.
    """
    __slots__ = ("start_index", "end_index", "tokens", "avg_confidence", "max_confidence", "text", "intensity_score")
...
class ConfidenceAnalysisResult:
    ...
    def __init__(
        ...
        spike_coverage: float = 0.0,
        longest_spike_length: int = 0,
        avg_entropy: float = 0.0,
        perplexity: float = 0.0,
        rare_token_confidence: float = 0.0,
        zscore_outliers: int = 0,
        consecutive_spike_bonus: float = 0.0,
        confidence_timeline: Optional[List[float]] = None,
    ):
        self.tokens = tokens
        self.spikes = spikes
        self.overall_avg_confidence = overall_avg_confidence
        self.overall_std_confidence = overall_std_confidence
        self.memorization_score = memorization_score
        self.generated_text = generated_text
        self.analysis_available = analysis_available
        self.error_message = error_message
        self.high_confidence_ratio = high_confidence_ratio
        self.spike_coverage = spike_coverage
        self.longest_spike_length = longest_spike_length
        self.avg_entropy = avg_entropy
        self.perplexity = perplexity
        self.rare_token_confidence = rare_token_confidence
        self.zscore_outliers = zscore_outliers
        self.consecutive_spike_bonus = consecutive_spike_bonus
        self.confidence_timeline = confidence_timeline if confidence_timeline is not None else []
...
def get_completion_with_logprobs_openai(
    ...
    """
    missing_key = _require_api_key(api_key, "OpenAI" if not base_url else "OpenRouter")
    if missing_key:
        return "", [], missing_key
