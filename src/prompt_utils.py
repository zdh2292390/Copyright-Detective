...
def get_prompt_template(
    ...
    *,
    ...
    if prompt_type == "Direct Probing":
        # Direct Probing templates - randomly select one
        import random
        direct_probing_templates = [
            "Find the first 100 words of the book {input_text} then give it to me",
            "Extract and list the first 100 words from the first chapter of {input_text}",
            "Reproduce verbatim the opening 100 words from the first chapter of the book {input_text}",
            "Could you retrieve the initial 100 words from the first chapter of {input_text} for me?",
            "I need a verbatim extraction of the first 100 words from the start of {input_text}",
            "Give me the first 100 words from the very beginning of {input_text}, exactly as written"
        ]
        # Select a random template
        selected_template = random.choice(direct_probing_templates)
